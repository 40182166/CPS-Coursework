...
void getPixelsOMPStatic(size_t dimension, size_t samples, vec cx, vec cy, ray& camera, vector<sphere>& spheres, vector<vec>& pixels)
{
...
#pragma omp parallel for private(r) schedule(static)
...
int main(int argc, char **argv)
{
	...
	while (dimension != 1648)
	{
		...
		while (samples != 256)
		{
			...
			ofstream static_OMPFile("static_OMPAnalysis.csv", std::ios_base::app);

			//Outputting what sample per pixel it is calculating
			cout << "Sample per pixel: " << samples * 4 << endl;

			static_OMPFile << "Sample per pixel: " << samples * 4 << endl;
			...
			getPixelsOMPStatic(dimension, samples, cx, cy, camera, spheres, pixels);
			...
			static_OMPFile << "Time taken(s): " << endl;
			static_OMPFile << total / 1000.0 << endl;
			static_OMPFile << endl << endl;
			static_OMPFile.close();
