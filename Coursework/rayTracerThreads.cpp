...
//Mutex to be used in the main loop of the algorithm
mutex mut;
		...
		for (size_t x = 0; x < dimension; ++x)
		{
			for (size_t sy = 0, i = (dimension - y - 1) * dimension + x; sy < 2; ++sy)
			{
				for (size_t sx = 0; sx < 2; ++sx)
				{
					...
					//Using mutex to prevent threads to access a resource at the same time
					//Using lock guard for safety to aout lock and unlock the mutex
...
int main(int argc, char **argv)
{
	...
	while (dimension != 1648)
	{
		...
		while (samples != 256)
		{
			...
			ofstream threadFile("threadAnalysis.csv", std::ios_base::app);

			//Outputting what sample per pixel it is calculating
			cout << "Sample per pixel: " << samples * 4 << endl;

			threadFile << "Sample per pixel: " << samples * 4 << endl;
			...
			//Creating vector to containt all threads
			vector<thread> allThreads;

			//Splitting dimension of the image by available threads
			auto range = dimension / num_threads;

			//Loop for creating threads based on a start and an end point
			for (int i = 0; i < num_threads - 1; i++)
			{
				//Creating Threads by setting the start and end point and pushing them to the vector
			...
			threadFile << "Time taken(s): " << endl;
			threadFile << total / 1000.0 << endl;
			threadFile << endl << endl;
			threadFile.close();
