...
void getPixelsOMPDynamic(size_t dimension, size_t samples, vec cx, vec cy, ray& camera, vector<sphere>& spheres, vector<vec>& pixels)
{
	...
	//Using OpenMP declaring r as private and scheduling the work dinamically
...
int main(int argc, char **argv)
{
	...
	//Starting loop to test algorithm on 2 different dimensions: 400 and 1024
	while (dimension != 1648)
	{

		size_t samples = 1; // Algorithm performs 4 * samples per pixel.

							//Starting loop to test algorithm on different samples per pixel
		while (samples != 256)
		{
			//Creating file to gather timings and append the result
			ofstream dynamic_OMPFile("dynamic_OMPAnalysis.csv", std::ios_base::app);

			//Outputting what sample per pixel it is calculating
			cout << "Sample per pixel: " << samples * 4 << endl;

			dynamic_OMPFile << "Sample per pixel: " << samples * 4 << endl;
			...
			vector<sphere> spheres
			{
				...
				////Spheres added for analysis on 14 shperes
				...
				////Spheres added for analysis on 20 spheres
				...
				////Spheres added for analysis on 25 spheres
				...
				//////******
				//
				////Spheres added for analysis on 30 spheres
			...
			//Calling method containing main loop and passing variables accordingly
			getPixelsOMPDynamic(dimension, samples, cx, cy, camera, spheres, pixels);

			//End point to measure time of execution
			auto end = system_clock::now();
			auto total = duration_cast<milliseconds>(end - start).count();

			//Outputting timing to the file
			dynamic_OMPFile << "Time taken(s): " << endl;
			dynamic_OMPFile << total / 1000.0 << endl;
			dynamic_OMPFile << endl << endl;
			dynamic_OMPFile.close();
			cout << "img.bmp" << (array2bmp("img.bmp", pixels, dimension, dimension) ? " Saved\n" : " Save Failed\n");

			//Multiplying current sample to get the next one
			samples *= 4;
		}

		//adding 624 to get 1024 dimension. When the loop reaches 1648, it stops
		dimension += 624;
	}
	return 0;
}