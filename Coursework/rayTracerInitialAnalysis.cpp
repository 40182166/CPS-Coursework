...
void oldGetPixels(size_t dimension, size_t samples, vec cx, vec cy, vec r, ray& camera, vector<sphere>& spheres, vector<vec>& pixels)
{
...
int main(int argc, char **argv)
{
	...
	while (dimension != 1648)
	{
		...
		while (samples != 256)
		{
			...
			ofstream initialFile("initialAnalysis.csv", std::ios_base::app);

			//Outputting what sample per pixel it is calculating
			cout << "Sample per pixel: " << samples * 4 << endl;

			initialFile << "Sample per pixel: " << samples * 4 << endl;
			...
			//Default algorithm without cout statements to guarantee correct timing
			...
			initialFile << "Time taken(s): " << endl;
			initialFile << total / 1000.0 << endl;
			initialFile << endl << endl;
			initialFile.close();
